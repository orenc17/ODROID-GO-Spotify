...
class SpotifyClientCredentials(object):
    ...
    def _request_access_token(self):
        ...
        response = urequests.post(self.OAUTH_TOKEN_URL, json=payload, headers=headers)
...
class SpotifyOAuth(object):
    ...
    def refresh_access_token(self, refresh_token):
        ...
        response = urequests.post(self.OAUTH_TOKEN_URL, data=dumped_data, headers=headers, json=None)
        if response.status_code != 200:
            self._warn("couldn't refresh token: code:%d reason:%s" % (response.status_code, response.reason))
            return None
        token_info = response.json()
        token_info = self._add_custom_values_to_token_info(token_info)
